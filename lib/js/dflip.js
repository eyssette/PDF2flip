	  ...
	  */ key: "getDocumentPageNumber",
			 ...
			 },
			 {
				  ...
				  value: function pinchUp(event) {
						if (event.touches != null && event.touches.length < 2 && this.pinchZoomDirty == true) {
							if(this.startTouches && distanceTouches && distanceTouches != 0) {
			 ...
			 },
			 {
				  ...
				  value: function pinchMove(event) {
					if (this.startTouches && event.touches != null && event.touches.length == 2){
						distanceTouches = getTouchesDistance(this.startTouches) - getTouchesDistance(event.touches)
					} else {
						distanceTouches = 0
					}
						if (event.touches != null && event.touches.length == 2 && this.startTouches != null) {
			 ...
			 },
			 {
				  ...
				  value: function mouseWheel(event) {
						var app = this.app, delta = base_viewer_utils.getMouseDelta(event);
	  ...
	  */ key: "mouseUp",
				  value: function mouseUp(event) {
						...
						if (viewer.app.zoomValue === 1) {
							 ...
							 var element = event.target || event.originalTarget; //check to see if the clicked element is a link, if so skip turn
							 var isClick = viewer.startPoint && point.x === viewer.startPoint.x && point.y === viewer.startPoint.y && element.nodeName !== "A";
							 if (event.ctrlKey === true && isClick || event.altKey === true && isClick) {
			...
			*/ function pdfLoaded(pdf) {
				 ...
				 */ pdf.getPage(2).then(function checkInternalPages(page) {
							 ...
							 if (app !== null && app.options != null) {
								  ...
								  app.updateInfo(cors + "<strong>Erreur: Problème d'accès au fichier!  </strong>" + fileName + "<br><br>" + error.message, "df-error");
								  console.log(error);
	  ...
	  */ key: "initInfo",
			 ...
			 },
			 {
				  ...
				  value: function zoom(delta, center) {
					...
					let zoom = Math.min(Math.max(1, app.viewer.camera.zoom + delta * 0.01), 4);
					...
					if (delta > 0 && zoom > 1 && zoom < 4) {
						 let deltaCenter = getDeltaPercentage(center, screenWidth, screenHeight);
