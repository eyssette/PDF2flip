// Ajouts pour faire fonctionner le zoom
...
const debounceZoom = debounce(function (zoom, point) {
  this.zoom(zoom, point);
}, 30);

function getDeltaPercentage(center, screenWidth, screenHeight) {
  let deltaX = center.x - screenWidth / 2;
  let deltaY = center.y - screenHeight / 2;

  let deltaXPercent = (deltaX / screenWidth) * 100;
  let deltaYPercent = (deltaY / screenHeight) * 100;

  return { x: deltaXPercent, y: deltaYPercent };
}

// Ajouts pour faire fonctionner le zoom sur mobile

let distanceTouches = 0;
...
function getTouchesDistance(touches) {
  ...
  return Math.sqrt(dx * dx + dy * dy); // Renvoie la distance
}
...
// Source: https://github.com/dearhive/3d-flipbook-dflip-lite/blob/main/assets/js/dflip.js
// Licence GPL : https://github.com/dearhive/3d-flipbook-dflip-lite?tab=GPL-3.0-1-ov-file#readme
...
/******/ (function () {
  ...
  !(function () {
    ...
    var BaseViewer = /*#__PURE__*/ (function () {
      ...
      _create_class(BaseViewer, [
          ...
          value: function pinchUp(event) {
            if (
              ...
              if (
                ...
                const isZoomOut = distanceTouches > 0 ? true : false;
                const point = { raw: getPinchCenter(this.startTouches) };
                this.zoomOnPoint(point, isZoomOut);
              } else {
        ...
        },
        {
          ...
          value: function pinchMove(event) {
            if (
              ...
              distanceTouches =
                getTouchesDistance(this.startTouches) -
                getTouchesDistance(event.touches);
            } else {
              distanceTouches = 0;
            }
            if (
              ...
              var newScale = base_viewer_utils.calculateScale(
                  this.startTouches,
                  base_viewer_utils.getTouches(event)
                ),
                scale = newScale / this.lastScale;
              this.lastScale = newScale;
              this.app.viewer.pinchZoomUpdateScale = base_viewer_utils.limitAt(
                newScale,
        ...
        },
        {
          ...
          value: function mouseWheel(event) {
            ...
            var isValidZoomOption =
              ...
              // La gestion des déplacements dans le PDF avec la souris est trop lente => désactivation tant qu'une autre solution n'a pas été trouvée
            const isActiveMouseWheelEvent = false

            if(isActiveMouseWheelEvent) {
              // Détection scroll horizontal ou vertical
              if (Math.abs(event.deltaX) > Math.abs(event.deltaY)) {
                if (event.deltaX > 0) {
                  app.viewer.camera.position.x += 10;
                  app.viewer.camera.updateProjectionMatrix();
                } else {
                  app.viewer.camera.position.x += -10;
                  app.viewer.camera.updateProjectionMatrix();
                }
              ...
              if (delta > 0) {
                app.viewer.camera.position.y += 10;
                app.viewer.camera.updateProjectionMatrix();
              } else if (delta < 0) {
                app.viewer.camera.position.y += -10;
                app.viewer.camera.updateProjectionMatrix();
              ...
              if (isValidZoomKey || isValidZoomOption) {
                if (delta > 0 || delta < 0) {
                  ...
                  const zoom = isZoomOut ? -30 : 30;
                  debounceZoom.call(app, zoom, point.raw);
                }
              } else if (
                app.options.mouseScrollAction ===
        ...
        {
          key: "zoomOnPoint",
          value: function zoomOnPoint(point, isZoomOut) {
            this.app.viewer.zoomCenter = point.raw;
            const zoom = isZoomOut ? -10 : 10;
            this.app.zoom(zoom, point.raw);
          },
        },
        {
          key: "getVisiblePages",
  ...
  */ var bodyG = sheet.element.geometry;
          ...
          value: function mouseUp(event) {
            ...
            if (viewer.app.zoomValue === 1) {
              ...
              var isClick =
                ...
                element.nodeName !== "A";
              if (
                (event.ctrlKey === true && isClick) ||
                (event.altKey === true && isClick)
              ) {
                const zoomOut = event.shiftKey;
                this.zoomOnPoint(point, zoomOut);
              } else if (
                isClick &&
                point.sheet &&
     ...
     */ var Page3D = /*#__PURE__*/ (function (Page2D) {
      ...
      return Page3D;
    })(Page2D); // CONCATENATED MODULE: ./src/js/dearviewer/viewers/viewers-lite.js
          ...
          value: function finalizeOutLine() {
            ...
            if (outline) {
              for (var count = 0; count < outline.length; count++) {
                ...
                outline[count].dest = outline[count].dest.replace(
                  /javascript:/g,
                  ""
                );
            ...
            pdfLoadProcess.promise
              ...
              .catch(function (error) {
                if (app !== null && app.options != null) {
                  ...
                  app.updateInfo(
                    cors +
                      "<strong>Erreur: Problème d'accès au fichier!  </strong>" +
                      fileName +
                      "<br><br>" +
                      error.message,
                    "df-error"
      ...
      app_create_class(App, [
        ...
        },
        {
          ...
          value: function initResourcesLocation() {
            ...
            if (
              ...
              options.pdfjsSrc =
                window[defaults_DEARVIEWER.locationVar] + "js/libs/pdf.min.js";
              options.threejsSrc =
        ...
        },
        {
          ...
          value: function zoom(delta, center) {
            ...
            let zoom = Math.min(
              Math.max(1, app.viewer.camera.zoom + delta * 0.01),
              4
            );
            app.viewer.camera.zoom = zoom;

            if (delta < 0) {
              app.viewer.zoomCenter = center;
            }

            if (delta > 0 && zoom > 1 && zoom < 4) {
              let deltaCenter = getDeltaPercentage(
                center,
                screenWidth,
                screenHeight
              );
              app.viewer.camera.position.x += deltaCenter.x;
              app.viewer.camera.position.y -= deltaCenter.y;
            } else {
              app.viewer.camera.position.x = 0;
              app.viewer.camera.position.y = 0;
            }
    ...
    /* globals jQuery, pdfjsLib, THREE */ function dflip_instanceof(
      left,
      right
    ) {
      if (
    ...
    var updateOptions = function updateOptions(options) {
      if (
        ...
        (Array === options.source.constructor ||
          Array.isArray(options.source) ||
          dflip_instanceof(options.source, Array))
      ) {
      ...
      if (options.pageSize !== void 0) {
        if (
          ...
          options.pageSize ===
          dflip_DEARFLIP.FLIPBOOK_PAGE_SIZE.DOUBLE_COVER_BACK
        ) {
        } else {
      ...
      if (options.singlePageMode !== void 0) {
        if (
          ...
          options.singlePageMode === 1 ||
          options.singlePageMode ===
            dflip_DEARFLIP.FLIPBOOK_SINGLE_PAGE_MODE.ZOOM
        ) {
          options.singlePageMode =
            dflip_DEARFLIP.FLIPBOOK_SINGLE_PAGE_MODE.ZOOM;
        } else if (
          ...
          options.singlePageMode === 2 ||
          options.singlePageMode ===
            dflip_DEARFLIP.FLIPBOOK_SINGLE_PAGE_MODE.BOOKLET
        ) {
          options.singlePageMode =
            dflip_DEARFLIP.FLIPBOOK_SINGLE_PAGE_MODE.BOOKLET;
        } else {
          options.singlePageMode =
            dflip_DEARFLIP.FLIPBOOK_SINGLE_PAGE_MODE.AUTO;
        }
      }
      ...
      if (options.direction !== void 0) {
        if (
          options.direction === "2" ||
          options.direction === 2 ||
          options.direction === dflip_DEARFLIP.READ_DIRECTION.RTL
        ) {
    ...
    /* harmony default export */ var dflip =
      /* unused pure expression or super */ null && dflip_DEARFLIP;
  })();
  /******/
})();