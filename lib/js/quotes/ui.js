...
export function displayQuotes(quotes) {
  ...
  quotes.forEach((quote) => {
    quoteList.insertAdjacentHTML(
      "beforeend",
      `
      <li class="quote-item flex items-center justify-between mb-2" data-id="${quote.id}">
...
export function toggleQuotePanel() {
  ...
  if ($("#quoteNotesPanel").hasClass("hidden")) {
    $("#toggleQuotePanelBtn").text("Mes notes");
  } else {
    $("#toggleQuotePanelBtn").text("Cacher les notes");
  }
}
...
export function closeQuotePanelOnClickOutside() {
  $(document).on("click", function (event) {
    if (
      !$(event.target).closest("#quoteNotesPanel").length &&
      !$(event.target).is("#toggleQuotePanelBtn")
    ) {
