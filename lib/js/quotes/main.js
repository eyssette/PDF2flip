...
document.addEventListener("click", function(event) {
  if (event.target.classList.contains("editQuoteBtn")) {
    ...
    store.get(id).onsuccess = function() {
      const quoteElement = document.querySelector(`.quote-item[data-id="${id}"]`)
...
document.getElementById("toggleQuotePanelBtn").addEventListener("click", toggleQuotePanel);
