...
function getPDF(pdfUrl,rtlMode, config) {
	let pageMode = config && config.pageMode ? config.pageMode : "AUTO";
		...
		hideOptions =
			config && config.hideOptions ? config.hideOptions : hideOptions;
...
function loadFlipbook(pdfUrl, rtlMode, config) {
  ...
  // S'il y a une erreur, on essaie de trouver la bonne URL, ou bien on utilise un proxy CORS
  .catch(() => {
    getPDF(handleURL(pdfUrl), rtlMode, config);
  });
}
